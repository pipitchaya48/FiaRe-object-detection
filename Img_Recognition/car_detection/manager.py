...
# crawler = GoogleImageCrawler(storage={'root_dir': 'car_images'})
# crawler.crawl(keyword='car accident damage side view', max_num=200)
...
model = YOLO("crashedCarDoor/crashedCarDoor.pt")
...
# Run YOLO detection (stream=True keeps results as generator for efficiency)
results = model(img, stream=True, classes=[0])  # class 2 = "car"
...
cv2.imshow("test1 result", img)
cv2.waitKey(0)
cv2.destroyAllWindows()

## can be made better by flipping img